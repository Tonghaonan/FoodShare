$(function () {
    ...
    $("#jqGrid").jqGrid({
        ...
        colNames:['id','用户名','创建时间','性别','生日','操作'],
        colModel: [
            //某些字段不要排序功能的话 只要在字段中加入 sortable：false；
            {label: 'id', name: 'uid', index: 'uid', hidden:true,width: 10, key: true},
            {label: '用户名', name: 'username', index: 'username', sortable: false, width: 30},
            {label: '创建时间', name: 'createTime', index: 'createTime', sortable: false, width: 50},
            {label: '性别', name: 'sex', index: 'sex', sortable: false, width: 10},
            {label: '生日', name: 'birthday', index: 'birthday', sortable: false, width: 50},
            {label: '操作', name: 'idDel', index: 'idDel', sortable: false, width: 50,
                formatter:function(cellValue, options, rowObject){
                    var pause_resume = "";
                    if ('1' == rowObject.isDel) {
                        pause_resume = '<button class="btn btn-info fa fa-plus" _type="product_lower" type="button">启用</button>  ';
                    } else if ('0' == rowObject.isDel){
                        pause_resume = '<button class="btn btn-danger fa fa-remove" _type="product_upper" type="button">禁用</button>  ';
                    }
                    return pause_resume;
                }
            }
